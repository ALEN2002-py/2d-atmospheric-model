...
| Integration time | up to $30\,\mathrm{min}$ |
...
| Scheme | $\Delta t$ | Steps | Wall time | Final $|w|_{\max}$ |
|---|---|---|---|---|
| RK4 | 0.01 s | 70 000 | 251 s | 0.220 m/s |
| SI | 15 s | 47 | 821 s | 0.220 m/s |
| EPI2 | 15 s | 47 | 83 s | 0.215 m/s |
| EPI3 | 15 s | 47 | 85 s | 0.216 m/s |
...
│
├── src/
│   ├── grid.py           # Unstaggered grid, isothermal base state, sponge layer
│   ├── dynamics.py       # compute_rhs / compute_linear_rhs / compute_nonlinear_rhs
│   │                     # Numba JIT for derivative kernels
│   ├── integrators.py    # All 7 schemes + robert_asselin_filter + _krylov_epi
│   └── results.py        # Save/load experiments (.npz + JSON sidecar)
│
├── experiments/
│   ├── gr_bubble.py        # Giraldo & Restelli (2008) Case 2 runner
│   ├── gr_efficiency.py    # Efficiency study: error vs wall time for G&R Case 2
│   ├── pc_bubble.py        # Pudykiewicz & Clancy (2022) Experiment 1 runner
│   ├── warm_bubble.py      # Robert (1993) small-domain runner + figures
...
│
├── docs/
│   └── equations.md      # Full LaTeX derivation
│
├── requirements.txt
└── README.md
```
...
python experiments/gr_bubble.py --compare       # all four + summary table
```
...
python experiments/gr_efficiency.py --plot_only  # replot from saved results
```
...
python experiments/pc_bubble.py --scheme EPI2 --t_end 1800
```
