...
-----
    ...
    python experiments/warm_bubble.py --scheme CTCS --dt 0.02 --n_steps 500
"""
...
def run_warm_bubble(scheme='CTCS', bubble_amp=2.0, dt=0.02,
                    n_steps=500, save=True):
    ...
    if not blown_up:
        print(f"  [{scheme}] Complete. "
              ...
              f"|θ'|_max={np.max(np.abs(state['theta'])):.3e}")
        if save:
            save_experiment(
...
def plot_bubble_snapshots(snapshots, grid, scheme, variable='theta'):
    ...
    cmap = 'hot_r' if variable == 'theta' else 'RdBu_r'
