...
ETD1V_CACHE  = os.path.join(RESULTS_DIR, "etd1v_dt_sweep_dx10m.json")
os.makedirs(OUT_DIR, exist_ok=True)
os.makedirs(RESULTS_DIR, exist_ok=True)

# ETD1V has no acoustic-CFL restriction (unconditionally stable linear part),
...
# shape of the curve, not just the long-dt regime. Because ETD1V's sub-step
...
ETD1V_DT_VALS = [0.02, 0.1, 0.5, 1.0, 2.0, 4.0, 8.0]
...
def run_etd1v(dt):
    ...
    t0 = wall_time.perf_counter()
    for _ in range(n_steps):
        state, _, _ = step(state, grid, dt, scheme="ETD1V")
...
def _run_etd1v_worker(dt):
    """Module-level (picklable) wrapper so each ETD1V dt runs in its own
    process via ProcessPoolExecutor -- the dt points are fully independent."""
    return dt, run_etd1v(dt)


def get_etd1v_results(skip_run):
    if os.path.exists(ETD1V_CACHE):
        with open(ETD1V_CACHE, "r", encoding="utf-8") as f:
            cached = json.load(f)
        print(f"  Loaded cached ETD1V results from {ETD1V_CACHE}")
        return cached
    if skip_run:
        print("  --skip-etd1v and no cache found -- ETD1V points will be omitted")
        return {}
    ...
    n_workers = min(len(ETD1V_DT_VALS), os.cpu_count() or 1)
    print(f"  Running {len(ETD1V_DT_VALS)} ETD1V dt value(s) across "
          f"{n_workers} worker process(es) in parallel...", flush=True)

    results = {}
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(_run_etd1v_worker, dt): dt for dt in ETD1V_DT_VALS}
        for fut in as_completed(futures):
            dt, r = fut.result()
            results[str(dt)] = r
    ...
    with open(ETD1V_CACHE, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print(f"  Cached ETD1V results -> {ETD1V_CACHE}")
    return results
...
def _collect_series(rk4_results, gmres_results, scheme, etd1v_results=None):
    """Return (dt_vals, wall_times, errors) sorted by dt, blown-up points excluded."""
    pts = []
    if scheme == "RK4":
        src = rk4_results
    elif scheme == "ETD1V":
        src = etd1v_results or {}
    else:
...
def make_frontier_plot(rk4_results, gmres_results, etd1v_results=None):
    ...
    styles = {
        ...
        "ETD1V": {"color": "#ff7f0e", "marker": "P"},
    }

    for i, scheme in enumerate(("RK4", "SI2", "ETD1V")):
        dts, walls, errs = _collect_series(rk4_results, gmres_results, scheme, etd1v_results)
        if not walls:
            continue
        s = styles[scheme]
...
def make_relative_time_plot(rk4_results, gmres_results, etd1v_results):
    """
    ...
    any_plotted = False
    for scheme in ("RK4", "SI2", "ETD1V"):
        dts, walls, _ = _collect_series(rk4_results, gmres_results, scheme, etd1v_results)
        if not walls:
            continue
        any_plotted = True
...
def make_error_vs_dt_plot(rk4_results, gmres_results, etd1v_results):
    """
    ...
    any_plotted = False
    for scheme in ("RK4", "SI2", "ETD1V"):
        dts, _, errs = _collect_series(rk4_results, gmres_results, scheme, etd1v_results)
        if not errs:
            continue
        any_plotted = True
        s = styles[scheme]
...
def print_summary_table(rk4_results, gmres_results, etd1v_results=None):
    print("\n" + "=" * 100)
    ...
    for dt_str, r in (etd1v_results or {}).items():
        _row("ETD1V", dt_str, r, kappa=None)
    print("=" * 100)
    ...
    if not any(gmres_results.get(s) for s in ("SI", "SI2")):
        ...
        print("  is (re-)run. Not an ETD1V-related gap.\n")


def main():
    parser = argparse.ArgumentParser(
        description="Real (measured) dt/wall-time/accuracy comparison: RK4 vs SI vs SI2 vs ETD1V"
    )
    parser.add_argument("--skip-rk4", action="store_true",
                        help="Don't run RK4 fresh; use cache only (omit if no cache)")
    parser.add_argument("--skip-etd1v", action="store_true",
                        help="Don't run ETD1V fresh; use cache only (omit if no cache)")
    args = parser.parse_args()

    gmres_results = load_gmres_cache()
    rk4_results   = get_rk4_results(args.skip_rk4)
    etd1v_results = get_etd1v_results(args.skip_etd1v)

    print_summary_table(rk4_results, gmres_results, etd1v_results)
    make_frontier_plot(rk4_results, gmres_results, etd1v_results)
    make_relative_time_plot(rk4_results, gmres_results, etd1v_results)
    make_error_vs_dt_plot(rk4_results, gmres_results, etd1v_results)
